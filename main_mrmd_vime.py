...
from dataset_mrmd_vime import (load_dataset, get_eval, mean_std,
...
if __name__ == '__main__':
    ...
    (train_X, test_X,
     ...
     # (decode_cat_from_embedding / decode_num_from_embedding). Dibutuhkan
     # karena decoder VIME yang di-freeze deterministik antar iterasi
     # diffusion — lihat header komentar dataset_mrmd_test_vime.py.
    ...
    for iteration in range(args.resume_iter, iter_end):
        ...
        mae, rmse, acc = get_eval(
            ...
            device        = device,
            oos           = False,
            bin_midpoints = bin_midpoints,    # untuk decode prediksi numerik (fallback)
            n_num_cols    = n_num_cols,
            num_true_norm = train_num,        # ground truth nilai asli ternormalisasi
            # [VIME] ref_* sudah di skala embedding asli (output VIME encoder),
            # sama seperti pred_X/X_true di atas — tidak perlu transformasi
            # tambahan. Mengaktifkan decode via Nearest-Neighbor (iterasi-aware).
            ref_embeddings = train_ref_emb,
            ref_all_idx    = train_ref_all_idx,
            ref_num_norm   = train_ref_num,
        )
        MAEs.append(mae)
        RMSEs.append(rmse)
        ACCs.append(acc)
        ...
        mae_out, rmse_out, acc_out = get_eval(
            ...
            # [VIME] ref dari train observed (proxy utk test) — lihat load_dataset
            ref_embeddings = test_ref_emb,
            ref_all_idx    = test_ref_all_idx,
            ref_num_norm   = test_ref_num,
        )
    ...
    if next_iter < args.max_iter:
        ...
        if not args.save_csv:
            ...
            test_csv_path = f'{imputed_csv_dir}/test_impute_mrmd_vime{last_iter}.csv'
            val_result_df = save_imputed_csv_mrmd(
                ...
                device        = device,
                oos           = True,
                # [VIME] konsisten dengan NN-decode yang dipakai get_eval OOS
                ref_embeddings = test_ref_emb,
                ref_all_idx    = test_ref_all_idx,
                ref_num_norm   = test_ref_num,
            )
            ...
            train_result_df = save_imputed_csv_mrmd(
                ...
                device        = device,
                oos           = False,
                # [VIME] konsisten dengan NN-decode yang dipakai get_eval in-sample
                ref_embeddings = train_ref_emb,
                ref_all_idx    = train_ref_all_idx,
                ref_num_norm   = train_ref_num,
            )
