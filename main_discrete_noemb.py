...
from dataset_discrete_noemb import (load_dataset, get_eval, mean_std,
                     decode_onehot_argmax,
                     save_imputed_csv_discrete_noemb, round_numeric_for_csv)
from diffusion_utils import sample_step, impute_mask

warnings.filterwarnings('ignore')

parser = argparse.ArgumentParser(description='Missing Value Imputation - Diskrit (MRmD) tanpa Embedding (one-hot)')
...
parser.add_argument('--noise_std',  type=float, default=0.00,       help='(Tidak dipakai - tidak ada model embedding pada varian ini, dipertahankan untuk kompatibilitas CLI).')
...
if __name__ == '__main__':
    ...
    #
    #  [VARIAN 1 - Diskrit tanpa Embedding] load_dataset mengembalikan:
    ...
    #    - onehot_sizes  : list[int] menggantikan emb_sizes
    #    - emb_model     : SELALU None (tidak ada embedding yang dilatih)
    #
    #  train_X / test_X : [N, total_onehot_dim]
    #    SEMUA fitur (numerik bin + kategorikal) sudah di-ONE-HOT (fixed
    #    representation, bukan hasil belajar). len_num = 0 karena tidak ada
    #    kolom raw numerik terpisah.
    # =========================================================================
    (train_X, test_X,
     ...
     cat_bin_num,
     emb_model,      # SELALU None pada varian ini
     onehot_sizes,   # [BARU] menggantikan emb_sizes
    ...
    assert emb_model is None, '[Sanity] Varian ini tidak boleh punya embedding model.'
    ...
    print(f'  - One-Hot Encoding (no emb): {t_emb:.4f}s')
    print(f'  - Total (Diskrit->OneHot)  : {t_total_preprocessing:.4f}s')
    print(f'{"="*60}')
    ...
    #
    #  Catatan: train_X sekarang hanya berisi one-hot (tidak ada raw numerik).
    #  extend_train_mask shape sudah cocok dengan train_X (total_onehot_dim).
    # =========================================================================
    ...
    # Seluruh isi train_X adalah one-hot (numerik bin + kategorikal).
    len_num = 0
    ...
    for iteration in range(args.resume_iter, iter_end):
        ...
        ckpt_dir = (f'ckpt/{dataname}_discrete_noemb/rate{ratio}/{mask_type}/'
        ...
        #
        #  Semua dimensi adalah one-hot (tidak ada raw numerik). Seluruh
        #  dimensi di-denorm ke skala one-hot asli untuk decoding.
        # =====================================================================
        ...
        mae, rmse, acc = get_eval(
            ...
            num_num       = len_num,          # selalu 0 di pipeline ini
            onehot_sizes  = onehot_sizes,
            mask          = ori_train_mask,
            device        = device,
            oos           = False,
        ...
        #  Simpan hasil
        # =====================================================================
        result_save_path = (f'results/{dataname}_discrete_noemb/rate{ratio}/{mask_type}/'
                            f'{split_idx}/{num_trials}_{num_steps}')
        ...
        with open(f'{result_save_path}/result_discrete_noemb.txt', 'a+', encoding='utf-8') as f:
            f.write(
            ...
            f.write(
                ...
                f'One-Hot encoding (no emb)={t_emb:.4f}s, '
                f'Total preprocessing (Diskrit+OneHot)={t_mrmd + t_emb:.4f}s\n'
            )
            f.write(
                f'iteration {iteration}, '
    ...
    if next_iter < args.max_iter:
        ...
        if not args.save_csv:
            ...
            # SELALU PAKAI ITERASI TERAKHIR (fixed, tidak bergantung pada
            # metrik test), konsisten dengan main_mrmd.py.
            ...
            val_result_df = save_imputed_csv_discrete_noemb(
                dataname      = dataname,
            ...
            train_result_df = save_imputed_csv_discrete_noemb(
                dataname      = dataname,
