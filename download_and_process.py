...
def train_val_split(dataname, ratio = 0.8):
    """
    Splits the EXISTING train.csv (the 70% training pool produced by
    train_test_split) again into 80% train / 20% val, using the exact same
    splitting logic as train_test_split (just applied on train.csv instead
    of data.csv).

    Results are saved in a separate `validation/` subfolder so they never
    mix with the main train.csv / test.csv:
        datasets/{dataname}/validation/train.csv
        datasets/{dataname}/validation/val.csv
    """
    data_dir = f'{DATA_DIR}/{dataname}'
    ...
    train_path = f'{val_dir}/train.csv'
    val_path = f'{val_dir}/val.csv'

    train_df.to_csv(train_path, index = False)
    val_df.to_csv(val_path, index = False)

    print(f'Spliting Training and Validation data for {dataname} is done.')
    print(f'Training data shape: {train_df.shape}, Validation data shape: {val_df.shape}')
    print(f'Training data saved at {train_path}, Validation data saved at {val_path} (kept separate from the main train.csv/test.csv).')
    

if __name__ == '__main__':

    # Set to True if you also want an 80:20 train/validation split carved out
    # of the 70% training pool (saved separately under `<dataset>/validation/`).
    USE_VALIDATION = True
    ...
    for name in NAME_URL_DICT_UCI.keys():
        ...
        # Step 1: 100% -> 70% train.csv / 30% test.csv (unchanged from before)
        train_test_split(name, ratio = 0.7, mask_prob = 0.3)

        # Step 2: split the 70% train.csv again into 80% train / 20% val,
        # saved separately under datasets/{name}/validation/
        if USE_VALIDATION:
        ...
        for mask_type in ['MCAR']:
            for mask_p in [0.3]:
                ...
                # Masks for the validation/train.csv / validation/val.csv,
                # saved under a separate masks/validation/... folder.
                if USE_VALIDATION:
                    generate_mask(dataname = name,
                                    mask_type = mask_type,
                                    mask_num = 10,
