...
from dataset_emb_nodiscrete import (load_dataset, get_eval, mean_std,
                     decode_cat_from_embedding,
                     save_imputed_csv_emb_nodiscrete, round_numeric_for_csv)
from diffusion_utils import sample_step, impute_mask

warnings.filterwarnings('ignore')

parser = argparse.ArgumentParser(description='Missing Value Imputation - Embedding (kategorikal) tanpa Diskritisasi MRmD')
...
parser.add_argument('--noise_std',  type=float, default=0.00,       help='Noise std for embedding model (kategorikal).')
...
parser.add_argument('--save_csv', type=str, default='True', choices=['True', 'False'],
                     help='True: simpan hasil imputasi (iterasi terakhir) ke CSV. False: tidak disimpan.')
...
if __name__ == '__main__':
    ...
    #
    #  [VARIAN 2 - Embedding tanpa Diskrit] load_dataset mengembalikan:
    #    - mrmd, bin_midpoints : SELALU None (tidak ada diskritisasi numerik)
    #    - n_num_cols          : int — jumlah kolom numerik KONTINU di awal train_X
    #    - emb_model, emb_sizes: HANYA untuk kolom kategorikal
    #
    #  train_X / test_X : [N, n_num_cols + total_cat_emb_dim]
    #    n_num_cols kolom PERTAMA = numerik kontinu (dinormalisasi, TIDAK
    #    di-embed). Sisanya = embedding hasil belajar untuk kategorikal.
    # =========================================================================
    (train_X, test_X,
     ori_train_mask, ori_test_mask,
     train_num, test_num,
     train_all_idx, test_all_idx,   # [N, n_cat_cols] — HANYA index kategorikal
     extend_train_mask, extend_test_mask,
     cat_bin_num,
     emb_model,
     emb_sizes,
     mrmd,           # SELALU None pada varian ini
     bin_midpoints,  # SELALU None pada varian ini
     n_num_cols,     # jumlah kolom numerik KONTINU di awal train_X
     t_mrmd,         # SELALU 0.0 (tidak ada diskritisasi)
     t_emb,          # waktu komputasi embedding training (kategorikal saja)
    ...
    assert mrmd is None and bin_midpoints is None, \
        '[Sanity] Varian ini tidak boleh melakukan diskritisasi MRmD.'

    t_total_preprocessing = t_mrmd + t_emb
    print(f'\n{"="*60}')
    print(f'[TIMING] Ringkasan Waktu Komputasi Preprocessing:')
    print(f'  - MRmD Discretization (dilewati) : {t_mrmd:.4f}s')
    print(f'  - Embedding Training (kategorik) : {t_emb:.4f}s')
    print(f'  - Total (Numerik-kontinu→Emb)    : {t_total_preprocessing:.4f}s')
    print(f'{"="*60}')

    # Setelah load_dataset selesai, aktifkan default device ke CUDA.
    torch.set_default_device(device)

    # Pindahkan emb_model ke GPU (jika ada kolom kategorikal)
    ...
    # =========================================================================
    #  Normalisasi
    #  mean & std dihitung pada observed entries,
    #  normalisasi: (X - mean) / std / 2
    #
    #  Catatan: train_X = [numerik_kontinu | embedding_kategorikal] digabung
    #  dulu, baru dinormalisasi ULANG di sini (sama seperti perlakuan pada
    #  embedding di dataset_mrmd.py) agar skala konsisten sebelum diffusion.
    # =========================================================================
    ...
    # len_num = 0 karena get_eval pada varian ini membaca numerik LANGSUNG
    # dari n_num_cols kolom pertama X_recon (bukan dari kolom raw terpisah
    # di luar train_X) -- konsisten dengan konvensi main_mrmd.py.
    len_num = 0
    ...
    for iteration in range(args.resume_iter, iter_end):
        ...
        ckpt_dir = (f'ckpt/{dataname}_emb_nodiscrete/rate{ratio}/{mask_type}/'
        ...
        #
        #  [MODIFIKASI] Numerik kontinu (n_num_cols kolom pertama) & embedding
        #  kategorikal SAMA-SAMA di-denorm dengan mean_X/std_X (normalisasi
        #  gabungan), lalu numerik langsung dipakai sebagai prediksi (tanpa
        #  decode bin), sedangkan kategorikal didecode lewat emb_model.
        # =====================================================================
        ...
        mae, rmse, acc = get_eval(
            ...
            device        = device,
            oos           = False,
            bin_midpoints = bin_midpoints,    # SELALU None
        ...
        #  Simpan hasil
        # =====================================================================
        result_save_path = (f'results/{dataname}_emb_nodiscrete/rate{ratio}/{mask_type}/'
                            f'{split_idx}/{num_trials}_{num_steps}')
        ...
        print(f'  - TOTAL (Numerik/Kategori→Imputasi): {t_total_pipeline:.4f}s')

        with open(f'{result_save_path}/result_emb_nodiscrete.txt', 'a+', encoding='utf-8') as f:
            f.write(
            ...
            f.write(
                f'iteration {iteration}, '
                f'MRmD discretization (dilewati)=0.0000s, '
                f'Embedding training (kategorik)={t_emb:.4f}s, '
                f'Total preprocessing={t_mrmd + t_emb:.4f}s\n'
            )
            f.write(
                f'iteration {iteration}, '
                f'TOTAL pipeline={t_total_pipeline:.4f}s\n\n'
            )
    ...
    if next_iter < args.max_iter:
        ...
        # =====================================================================
        #  [BARU] Simpan hasil imputasi (iterasi TERAKHIR) ke CSV.
        # =====================================================================
        if not args.save_csv:
            ...
            # SELALU PAKAI ITERASI TERAKHIR (fixed), konsisten dengan main_mrmd.py.
            ...
            test_csv_path = f'{imputed_csv_dir}/test_impute_iter{last_iter}.csv'
            val_result_df = save_imputed_csv_emb_nodiscrete(
                dataname      = dataname,
            ...
            train_result_df = save_imputed_csv_emb_nodiscrete(
                dataname      = dataname,
