...
from dataset_caimwith2 import (load_dataset, get_eval, mean_std,
...
if __name__ == '__main__':
    ...
    #    - caim          : CAIMDiscretizer (untuk decode numerik)
    ...
    (train_X, test_X,
     ...
     emb_model,
     emb_sizes,
     caim,           # [BARU] CAIMDiscretizer (atau None jika tidak ada numerik)
    ...
    t_total_preprocessing = t_caim + t_emb
    print(f'\n{"="*60}')
    print(f'[TIMING] Ringkasan Waktu Komputasi Preprocessing:')
    print(f'  - CAIM Discretization : {t_caim:.4f}s')
    ...
    for iteration in range(args.resume_iter, iter_end):
        ...
        t_total_pipeline = t_caim + t_emb + t_train + t_impute_in + t_impute_out
        ...
        with open(f'{result_save_path}/result_caimwith.txt', 'a+', encoding='utf-8') as f:
            f.write(
            ...
            f.write(
                f'iteration {iteration}, '
                f'CAIM discretization={t_caim:.4f}s, '
                f'Embedding training={t_emb:.4f}s, '
                f'Total preprocessing (Diskrit+Emb)={t_caim + t_emb:.4f}s\n'
            )
            f.write(
                f'iteration {iteration}, '
