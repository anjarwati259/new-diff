...
# ===========================================================================
#  VIME Self-Supervised Encoder — MENGGANTIKAN PT-VAE
#  Berdasarkan: Yoon, J., Zhang, Y., Jordon, J., & van der Schaar, M. (2020).
#  "VIME: Extending the Success of Self- and Semi-supervised Learning to
#  Tabular Domain." NeurIPS 2020. https://papers.nips.cc/paper/2020/hash/
#  7d97667a3e056acab9aaf653807b4a03-Abstract.html
#  Implementasi resmi: https://github.com/jsyoon0823/VIME
#
#  Arsitektur (Section 4.1, Fig. 1, Eq. 3-6):
#    Encoder e            : x → z            (Linear→ReLU→Linear)
#    Feature Estimator sr  : z → x̂ (rekonstruksi fitur)   (Linear→ReLU→Linear)
#    Mask Estimator sm     : z → m̂ (estimasi mask korupsi) (Linear→ReLU→Linear→Sigmoid)
#    Corrupt (Eq. 3): x̃ = m⊙x̄ + (1-m)⊙x, m_j~Bernoulli(p_m), x̄=empirical marginal
#    Loss (Eq. 4-6)  : L = lm(m, m̂) + α·lr(x, x̂)
#                       lm = BCE per kolom (mask estimation)
#                       lr = MSE (numerik) / CrossEntropy (kategorikal, sesuai
#                            catatan resmi paper Section 4.1)
#
#  Input berbasis ONE-HOT (seperti DAE, BUKAN nn.Embedding lookup seperti
#  PT-VAE), dan output SATU vektor laten global per sampel (hidden_dim),
#  BUKAN concat per kolom seperti PT-VAE — karena encoder VIME memproses
#  seluruh vektor one-hot gabungan sekaligus.
#
#  [DECODE — dipertahankan sesuai reference dataset_mrmdwith_vime.py]
#  decode_cat_from_embedding / decode_num_from_embedding memakai Nearest-
#  Neighbor (NN) di ruang laten z sebagai mode utama (bukan frozen decoder
#  VIME), karena decoder VIME yang di-freeze menghasilkan output yang sama
#  persis di setiap iterasi diffusion (tidak "iterasi-aware"). NN-decode
#  mencari z_ref (dari sampel fully-observed) paling dekat dan memakai
#  label/nilainya sebagai prediksi — sehingga hasil berubah mengikuti
#  pred_X yang berubah tiap iterasi diffusion. Mode frozen-decoder tetap
#  tersedia sebagai fallback jika ref_embeddings tidak diberikan.
#
#  [FIX-LEAKAGE — KHUSUS FILE INI, TIDAK ADA DI VERSI VIME DASAR]
#  Dataset di sini punya nilai yang MEMANG hilang (mask_array, real missing
#  value untuk task imputasi). Karena VIME berbasis one-hot (seperti DAE),
#  mekanisme anti-leakage-nya SAMA seperti DAE:
#
#    1) HARD MASKING (deterministik): posisi yang BENAR-BENAR missing
#       di-slice one-hot kolomnya menjadi NOL SEBELUM masuk ke encoder —
#       diterapkan permanen, sebelum corrupt() self-supervised VIME sendiri
#       dijalankan. Encoder TIDAK PERNAH melihat nilai asli di posisi itu.
#    2) Corrupt() self-supervised VIME (Bernoulli p_m, Eq. 3) HANYA
#       menyasar kolom yang BENAR-BENAR observed — kolom yang sudah
#       di-hard-mask dilewati (tidak "dobel di-corrupt").
#    3) Reconstruction loss lr (MSE/CE, komponen dari Eq. 4) HANYA
#       dihitung pada posisi OBSERVED per kolom — posisi missing
#       DIKECUALIKAN, sama seperti prinsip fix-leakage pada DAE/PT-VAE.
#    4) Loss lm (mask estimation, Eq. 5) TIDAK di-mask — targetnya (m_col)
#       sudah otomatis dipaksa 0 pada kolom yang di-hard-mask (karena tidak
#       ikut di-corrupt), sehingga tidak ada risiko leakage di situ.
#
#  [PENTING] Ref-embeddings (train_ref_emb, dll.) untuk NN-decode TETAP
#  dibangun dari sampel TRAIN_FULL yang FULLY-OBSERVED (tidak ada kolom
#  missing sama sekali), konsisten dengan cara load_dataset menghasilkan
#  train_all_mask di pipeline fix-leakage ini.
#
#  Fitur numerik (hasil diskritisasi MRmD) tetap di-embed BERSAMA fitur
#  kategorikal memakai model VIME yang sama. Pipeline MRmD → normalisasi →
#  diffusion → imputasi TIDAK BERUBAH.
# ===========================================================================
...
class VIMEEmbeddingModel(nn.Module):
    """
    VIME Self-Supervised Encoder — MENGGANTIKAN PTVAEEmbeddingModel.

    Sesuai paper NeurIPS 2020 (Yoon et al., Section 4.1) dan implementasi
    resmi GitHub jsyoon0823/VIME.
    ...
    Komponen utama:
      ...
      - Mask Estimator sm   : hidden_dim → hidden_dim → n_cols     (Linear→ReLU→Linear→Sigmoid)

    Input:
      x_idx [batch, n_cols] int → one-hot [batch, input_dim] float (di dalam model)

    Corrupt generation (Eq. 3):
      x_tilde = m ⊙ x_bar + (1-m) ⊙ x
      m_j ~ Bernoulli(p_m) per KOLOM (bukan per dimensi one-hot)
      x_bar[j] ~ empirical marginal (row-shuffle dalam batch)

    Loss (Eq. 4-6):
      L = lm(m, m_hat) + alpha * lr(x, x_hat)
      lm = BCE per kolom (mask estimation loss, Eq. 5)
      lr = MSE per kolom untuk numerik, CrossEntropy per kolom untuk
           kategorikal (paper: "For categorical variables, we modified
           Equation 6 to cross-entropy loss.")

    [FIX-LEAKAGE — tambahan dibanding dataset_mrmdwith_vime.py]
    forward() & encode_from_idx() menerima parameter opsional
    `observed_mask` [batch, n_cols] bool (True = OBSERVED). Jika diberikan:
      - Slice one-hot kolom pada posisi missing di-NOL-kan PERMANEN sebelum
        corrupt() self-supervised dijalankan (encoder tidak pernah melihat
        nilai asli di posisi itu).
      - corrupt() TIDAK menambah corruption stokastik pada kolom yang sudah
        di-hard-mask (skip_mask), supaya tidak "dobel di-corrupt" dan agar
        target m_col tetap 0 (bukan "corrupted") di posisi itu — valid,
        tidak leakage.
    ...
    Untuk kompatibilitas dengan pipeline decode (get_eval):
      ...
      - decode(z)      : z [N, hidden_dim] → list[n_cols] logits
                         ...
                         (mode fallback deterministik; mode utama evaluasi
                         memakai Nearest-Neighbor, lihat decode_cat_from_embedding)
    """
    ...
    def __init__(self, all_dims: list, hidden_dim: int, p_m: float = 0.3,
        ...
        # Paper Eq. 5 & Figure 1: sm: Z → [0,1]^d, d = n_cols (satu bit per KOLOM)
        self.mask_estimator = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, self.n_cols),
            nn.Sigmoid(),
        )
    ...
    def _apply_missing_zero(self, x_onehot: torch.Tensor,
                            missing_mask: torch.Tensor) -> torch.Tensor:
        """
        [FIX-LEAKAGE] Nol-kan slice one-hot kolom pada posisi yang BENAR-BENAR
        missing, SEBELUM corrupt() self-supervised dijalankan. Deterministik.

        x_onehot     : [batch, input_dim]
        missing_mask : [batch, n_cols] bool — True = missing (bukan observed)
        """
        x_onehot = x_onehot.clone()
        for j in range(self.n_cols):
            rows = missing_mask[:, j]
            if rows.any():
                s = self._offsets[j]
                e = self._offsets[j + 1]
                x_onehot[rows, s:e] = 0.0
        return x_onehot

    # ── Corrupt generation (Eq. 3, paper Section 4.1) ─────────────────────

    def corrupt(self, x_onehot: torch.Tensor, skip_mask: torch.Tensor = None) -> tuple:
        """
        ...
        skip_mask : [batch, n_cols] bool, opsional — True = JANGAN corrupt
            ...
            di-hard-mask permanen oleh _apply_missing_zero — FIX-LEAKAGE).
            m_col dipaksa 0 di posisi ini (bukan "corrupted").
        ...
        # Mask per KOLOM sesuai paper: mj ~ Bernoulli(pm)
        m_col = torch.bernoulli(
            torch.full((batch_size, self.n_cols), self.p_m, device=device)
        )  # [batch, n_cols]

        if skip_mask is not None:
            # [FIX-LEAKAGE] Jangan corrupt kolom yang sudah di-hard-mask —
            # paksa m_col=0 di posisi itu (valid, bukan leakage: artinya
            # "kolom ini tidak di-corrupt tambahan", yang memang benar).
            m_col = m_col * (~skip_mask).float()
    ...
    def encode(self, x_float: torch.Tensor) -> torch.Tensor:
        ...
        x_float : [batch, input_dim]  (one-hot float)
        return  : [batch, hidden_dim]
        """
        return self.encoder(x_float)

    def encode_from_idx(self, x_idx: torch.Tensor,
        ...
        observed_mask : [batch, n_cols] bool, opsional — True = observed.
            ...
            encode. Untuk encode_with_embedding() pada data ground-truth,
            biarkan None (lihat komentar di load_dataset).
        ...
        if observed_mask is not None:
            x_onehot = self._apply_missing_zero(x_onehot, ~observed_mask)
        return self.encoder(x_onehot)

    def decode(self, z: torch.Tensor) -> list:
        """
        Decode latent z → logits per kolom (mode fallback deterministik).
    ...
    def forward(self, x_idx: torch.Tensor,
               observed_mask: torch.Tensor = None) -> tuple:
        """
        Forward pass lengkap: (hard-mask) → corrupt → encode → estimasi mask & fitur.
        ...
        return: (z, m_hat, x_hat_flat, m_col, x_onehot)
          ...
          x_onehot   : [batch, input_dim]   — input asli one-hot (SEBELUM hard-mask —
                       dipakai sbg target reconstruction loss, HANYA pada posisi
                       observed, lihat train_supervised_embedding_model)
        """
        x_onehot_true = self.idx_to_onehot(x_idx)      # [batch, input_dim] — nilai ASLI

        missing_mask = (~observed_mask) if observed_mask is not None else None
        if missing_mask is not None:
            x_onehot_in = self._apply_missing_zero(x_onehot_true, missing_mask)
        else:
            x_onehot_in = x_onehot_true

        x_tilde, m_col = self.corrupt(x_onehot_in, skip_mask=missing_mask)

        z          = self.encoder(x_tilde)            # [batch, hidden_dim]
        m_hat      = self.mask_estimator(z)            # [batch, n_cols]
        x_hat_flat = self.feature_estimator(z)        # [batch, input_dim]

        return z, m_hat, x_hat_flat, m_col, x_onehot_true


# Alias kompatibilitas — beberapa docstring/anotasi tipe di file ini masih
# mereferensikan nama lama (SupervisedLearnableEmbeddingModel, DAEEmbeddingModel,
# VAEEmbeddingModel, PTVAEEmbeddingModel dari versi-versi sebelumnya).
SupervisedLearnableEmbeddingModel = VIMEEmbeddingModel
DAEEmbeddingModel = VIMEEmbeddingModel
VAEEmbeddingModel = VIMEEmbeddingModel
PTVAEEmbeddingModel = VIMEEmbeddingModel


# ===========================================================================
#  Training VIME Self-Supervised Encoder — MENGGANTIKAN Training PT-VAE
#  Berdasarkan train_supervised_embedding_model pada dataset_mrmdwith_vime.py,
#  DITAMBAH mekanisme fix-leakage (hard-mask one-hot + loss ter-mask) —
#  PERSIS pola yang dipakai versi DAE (karena VIME juga berbasis one-hot,
#  BUKAN nn.Embedding lookup seperti PT-VAE).
# ===========================================================================

def train_supervised_embedding_model(cat_idx_array: np.ndarray,
                                     labels: np.ndarray,          # diterima tapi TIDAK dipakai (VIME self-supervised, tanpa label)
                                     cat_dims: list,
                                     emb_sizes: list,             # diterima tapi TIDAK dipakai utk dimensi (lihat VIMEEmbeddingModel)
                                     n_classes: int,              # [KONVENSI VIME] dipakai sbg proxy n_num_cols_for_loss (lihat load_dataset)
                                     device: str,
                                     n_epochs: int = 50,
                                     batch_size: int = 1024,
                                     lr: float = 1e-3,
                                     dropout: float = 0.1,        # diabaikan (bukan arsitektur paper VIME)
                                     hidden_dim: int = 256,
                                     use_mlp: bool = True,        # diabaikan
                                     mlp_ratio: float = 1.5,      # diabaikan
                                     noise_std: float = 0.01,     # diabaikan, dipertahankan utk kompatibilitas signature
                                     patience: int = 30,
                                     mask_array: np.ndarray = None,
                                     p_m: float = 0.3,
                                     alpha: float = 1.0) -> VIMEEmbeddingModel:
    """
    Latih VIME self-supervised encoder (Yoon et al., 2020).

    Signature dipertahankan agar load_dataset tidak perlu mengubah cara
    pemanggilan. Parameter yang tidak relevan untuk VIME (dropout, use_mlp,
    mlp_ratio, noise_std) diterima tapi diabaikan.

    [KONVENSI DIPERTAHANKAN dari dataset_mrmdwith_vime.py] n_classes dipakai
    sebagai proxy n_num_cols_for_loss (jumlah kolom pertama di cat_idx_array
    yang merupakan numerik-bin, sisanya kategorikal) — caller (load_dataset)
    meneruskan n_num_cols lewat parameter n_classes ini.
    ...
    mask_array : [N, n_cols] bool, opsional — True = nilai HILANG (missing).
        [FIX-LEAKAGE] Jika diberikan, posisi missing:
          1) Di-hard-mask (one-hot kolomnya di-nol-kan) SEBELUM corrupt()
             self-supervised VIME dijalankan — pengganti langsung mekanisme
             DAE (karena arsitektur input sama-sama one-hot).
          2) corrupt() TIDAK menambah corruption stokastik pada kolom yang
             sudah di-hard-mask (skip_mask) — m_col dipaksa 0 di posisi itu.
          3) DIKECUALIKAN dari reconstruction loss lr (MSE/CE) per kolom.
        Loss lm (mask estimation) TIDAK di-mask — m_col sudah otomatis 0
        di posisi missing (bukan "corrupted"), jadi tidak ada leakage.
    """
    # ── Hitung hidden_dim ────────────────────────────────────────────────
    ...
    # [Konvensi dipertahankan] diteruskan lewat parameter n_classes oleh caller.
    n_num_cols_for_loss = n_classes
    ...
    mse_loss_noreduce = nn.MSELoss(reduction='none')      # lr numerik: MSE per-elemen (Eq. 6)
    ...
    print(f'[VIME] p_m={p_m}, alpha={alpha}, '
    ...
    for epoch in range(n_epochs):
        ...
        for batch in loader:
            ...
            # Forward: (hard-mask) → corrupt → encode → estimasi mask & fitur
            z, m_hat, x_hat_flat, m_col, x_onehot = model(batch_cat, observed_mask=batch_observed)

            # ── Loss lm: mask estimation BCE (Paper Eq. 5) ───────────────
            # TIDAK di-mask — m_col sudah otomatis 0 di kolom hard-masked.
            loss_m = bce_loss(m_hat, m_col)

            # ── Loss lr: feature reconstruction (Paper Eq. 6 + catatan) ──
            # [FIX-LEAKAGE] target SELALU x_onehot (nilai ASLI, dikembalikan
            # oleh forward() SEBELUM hard-mask), dan HANYA dihitung pada
            # posisi OBSERVED per kolom.
            ...
            for j in range(model.n_cols):
                ...
                x_true_j = x_onehot[:, s:e]      # [batch, vocab_j]  (one-hot float, ASLI)

                if j < n_num_cols_for_loss:
                    # Numerik (bin): MSE atas one-hot representasi, per-sampel
                    per_elem_j = mse_loss_noreduce(x_hat_j, x_true_j).mean(dim=1)  # [batch]
                else:
                    # Kategorikal: cross-entropy per-sampel
                    target_j   = x_true_j.argmax(dim=1)   # [batch] int
                    per_elem_j = ce_loss_noreduce(x_hat_j, target_j)  # [batch]

                if batch_observed is not None:
                    obs_j = batch_observed[:, j]
                    if obs_j.any():
                        col_losses.append(per_elem_j[obs_j].mean())
                else:
                    col_losses.append(per_elem_j.mean())

            loss_r = (sum(col_losses) / len(col_losses)) if len(col_losses) > 0 \
        ...
        if (epoch + 1) % 10 == 0:
            print(f'[VIME] Epoch {epoch+1}/{n_epochs} - '
                  f'Loss: {avg_loss:.4f} '
                  f'(Mask: {avg_mask_loss:.4f}, Feat(obs-only): {avg_feat_loss:.4f})')
    ...
    with torch.no_grad():
        ...
        # encode_from_idx() ground-truth: TANPA observed_mask, karena dipakai
        # hanya utk memantau distribusi representasi, bukan training/imputasi.
...
# ===========================================================================
#  Encode / Decode helpers (disesuaikan ke VIMEEmbeddingModel)
#  [DIPERTAHANKAN dari dataset_mrmdwith_vime.py] decode_cat_from_embedding /
#  decode_num_from_embedding memakai Nearest-Neighbor (NN) di ruang laten z
#  sebagai mode utama, karena decoder VIME yang di-freeze menghasilkan
#  output DETERMINISTIK (sama persis tiap iterasi diffusion). NN-decode
#  mencari z_ref (dari sampel fully-observed) paling dekat dan memakai
#  label/nilainya — sehingga hasil "iterasi-aware" mengikuti pred_X yang
#  berubah tiap iterasi. Mode frozen-decoder (fallback) tetap tersedia jika
#  ref_embeddings tidak diberikan.
# ===========================================================================
...
def encode_with_embedding(model: VIMEEmbeddingModel,
    ...
    dipakai sebagai target evaluasi imputasi & referensi NN-decode, bukan
    sebagai sinyal training embedding. Fix-leakage hanya relevan saat
    TRAINING model embedding (lihat train_supervised_embedding_model).
...
def decode_cat_from_embedding(model: VIMEEmbeddingModel,
    ...
    Mode 1 — Nearest-Neighbor di z-space (DEFAULT, dipakai saat ref tersedia):
        ...
        (L2 distance) dari ref_embeddings (embedding sampel fully-observed).
        Prediksi = label dari z_ref terdekat. Iterasi-aware: hasil diffusion
        yang berbeda tiap iterasi menghasilkan z berbeda → neighbor berbeda.

    Mode 2 — VIME frozen decoder (FALLBACK, jika ref tidak tersedia):
        Menggunakan feature_estimator sebagai decoder. DETERMINISTIK.

    Parameter
    ---------
    emb_array       : [N, hidden_dim]  — embedding hasil diffusion (di-denorm)
    ref_embeddings  : [N_ref, hidden_dim]  — embedding referensi (fully-observed)
...
def decode_num_from_embedding(model: VIMEEmbeddingModel,
    ...
    Mode 1 — Nearest-Neighbor di z-space (DEFAULT, dipakai saat ref tersedia):
        ...
        ref_embeddings (embedding fully-observed). Prediksi nilai numerik =
        nilai asli ternormalisasi dari NN tersebut. Iterasi-aware.
    ...
    n_num_cols     : int
    ref_embeddings : [N_ref, hidden_dim]  — embedding referensi (fully-observed)
    ref_num_norm   : [N_ref, n_num_cols]  — nilai numerik ternormalisasi (referensi)
...
def load_dataset(dataname, idx=0, mask_type='MCAR', ratio='30', noise_std=0.01):
    ...
    vime_hidden_dim = 256
    emb_sizes = [vime_hidden_dim] * len(all_dims)   # placeholder per kolom (kompatibilitas signature)
    ...
    # ── Latih VIMEEmbeddingModel LANGSUNG di TRAIN_FULL ────────────────────
    ...
    #
    # [VIME — menggantikan PT-VAE] Berbeda dengan PT-VAE (nn.Embedding lookup,
    # output per-kolom concat), VIME berbasis ONE-HOT (seperti DAE) dan
    # menghasilkan SATU vektor laten global (hidden_dim) per sampel. mask_array
    # (train_all_mask) dipakai untuk fix-leakage via HARD-MASK one-hot
    # (bukan token missing seperti PT-VAE) + exclude posisi missing dari
    # komponen reconstruction loss lr.
    #
    # [KONVENSI VIME] n_num_cols diteruskan lewat parameter n_classes karena
    # signature train_supervised_embedding_model dipertahankan agar caller
    # (load_dataset) tidak perlu berubah — lihat dataset_mrmdwith_vime.py.
    print('[VIME] Melatih VIME self-supervised encoder '
          '(mask estimation + feature reconstruction loss, fix-leakage) LANGSUNG pada TRAIN_FULL ...')
    print('[VIME] Referensi: Yoon et al. (2020) VIME, NeurIPS 2020 — github.com/jsyoon0823/VIME')
    ...
    emb_model = train_supervised_embedding_model(
        ...
        labels        = full_train_labels, # DIABAIKAN oleh VIME (self-supervised), diteruskan demi kompatibilitas
        cat_dims      = all_dims,
        emb_sizes     = emb_sizes,         # placeholder, DIABAIKAN utk dimensi (lihat VIMEEmbeddingModel)
        n_classes     = n_num_cols,        # [KONVENSI VIME] dipakai sbg n_num_cols_for_loss, BUKAN jumlah kelas
        device        = device,
        n_epochs      = 1000,
        batch_size    = 1024,
        lr            = 1e-3,
        ...
        use_mlp       = True,              # DIABAIKAN
        mlp_ratio     = 1.5,               # DIABAIKAN
        noise_std     = noise_std,         # DIABAIKAN oleh VIME, dipertahankan demi kompatibilitas signature
        patience      = 40,
        mask_array    = train_all_mask,    # [FIX-LEAKAGE] mask TRAIN_FULL — posisi missing di-hard-mask + dikecualikan dari loss lr
        p_m           = 0.3,               # probabilitas masking Bernoulli (paper default)
        alpha         = 1.0,               # bobot feature reconstruction loss (Eq. 4)
    )
    t_emb_end = time.time()
    ...
    # ── Encode TRAIN_FULL & TEST → representasi VIME z ─────────────────────
    ...
    # karena keduanya dipakai sebagai ground-truth (X_true) & referensi
    # NN-decode, BUKAN sebagai sinyal training embedding. Fix-leakage sudah
    ...
    # ── [VIME] Perluas mask [N, n_cols] → [N, hidden_dim] ──────────────────
    # VIME encoder menghasilkan SATU vektor laten global per sampel — sampel
    # dengan SETIDAKNYA SATU kolom missing menghasilkan embedding yang
    # "terkontaminasi" (kolom missing sudah di-hard-mask sebelum encode).
    # Karena itu status "missing" ditandai di SELURUH hidden_dim untuk
    # sampel tersebut (bukan per-dimensi), mengikuti dataset_mrmdwith_vime.py.
    ...
    # ── [VIME] Simpan embedding fully-observed sebagai referensi NN-decode ──
    # train_ref_emb/train_ref_all_idx/train_ref_num: dari sampel TRAIN_FULL
    # yang TIDAK punya kolom missing sama sekali (fully-observed), dipakai
    # oleh get_eval sebagai ref_embeddings untuk decode_cat/num_from_embedding
    # (Nearest-Neighbor di z-space, iterasi-aware — lihat header komentar file).
    ...
    if train_obs_mask.sum() > 0:
        train_ref_emb     = train_all_emb[train_obs_mask]
        train_ref_all_idx = train_all_idx[train_obs_mask]
        train_ref_num     = train_num[train_obs_mask] if n_num_cols > 0 else None
    else:
        # Fallback: pakai semua training data jika semua sampel ada missing
        train_ref_emb     = train_all_emb
        train_ref_all_idx = train_all_idx
        train_ref_num     = train_num if n_num_cols > 0 else None

    # Referensi untuk OOS (test) decode: proxy dari train observed (test
    # adalah data yang ingin diprediksi, jadi tidak bisa dipakai sbg ref sendiri)
    test_ref_emb     = train_ref_emb
    test_ref_all_idx = train_ref_all_idx
    test_ref_num     = train_ref_num

    print(f'[VIME][Ref] Train ref samples (fully-observed): {train_ref_emb.shape[0]}')
    print(f'[VIME][Ref] Test  ref samples (dari train obs): {test_ref_emb.shape[0]}')
    ...
    return (train_X, test_X,
            ...
            # ─────────────────────────────────────────────────────────────
            # [VIME — BARU] Referensi embedding fully-observed utk NN-decode
            # (decode_cat_from_embedding / decode_num_from_embedding).
            train_ref_emb,      # embedding referensi in-sample (fully-observed)
            train_ref_all_idx,  # label index referensi in-sample
            train_ref_num,      # nilai numerik referensi in-sample (norm), atau None
            test_ref_emb,       # embedding referensi OOS (dari train observed)
            test_ref_all_idx,   # label index referensi OOS
            test_ref_num,       # nilai numerik referensi OOS (norm), atau None
            # ─────────────────────────────────────────────────────────────
...
def get_eval(dataname, X_recon, X_true, truth_all_idx,
    ...
    [DIPERBARUI untuk VIME] Tambahan parameter ref_embeddings/ref_all_idx/
    ref_num_norm (default None, backward compatible dengan DAE & PT-VAE)
    untuk mengaktifkan decode via Nearest-Neighbor di z-space, sesuai
    dataset_mrmdwith_vime.py — karena decoder VIME frozen menghasilkan
    output deterministik (sama tiap iterasi diffusion) jika TIDAK dipakai.

    Alur decode dengan ref (VIME, iterasi-aware):
      X_recon (embedding diffusion, skala asli)
        → cari NN di ref_embeddings (embedding sampel fully-observed)
        → ambil label/nilai dari NN tersebut
        → bandingkan dengan ground truth (truth_all_idx / num_true_norm)

    Jika ref_embeddings=None (mis. dipanggil dari pipeline DAE/PT-VAE lama),
    decode_cat_from_embedding/decode_num_from_embedding otomatis jatuh ke
    mode frozen-decoder (fallback) — perilaku identik dengan sebelumnya.
    ...
    bin_midpoints  : list[n_num_cols] of np.ndarray  — midpoint per bin, skala norm
                     (dipakai sbg fallback jika ref tidak tersedia)
    ...
    if (n_num_cols > 0
        ...
        # Decode via NN di z-space jika ref tersedia (VIME, iterasi-aware),
        # jika tidak → fallback ke bin_midpoints (frozen decoder).
    ...
    # ── Kategorikal: Akurasi via NN di z-space (atau fallback decoder) ─────
    acc = np.nan
    if (truth_all_idx is not None
        ...
        # Decode semua kolom → predicted index (NN jika ref tersedia)
...
def _decode_and_denormalize_numeric(pred_X_emb, emb_model, bin_midpoints,
                                    ...
                                    ref_embeddings=None, ref_num_norm=None):
    """
    ...
    ref_embeddings, ref_num_norm : opsional — [VIME] jika diberikan, decode
        memakai Nearest-Neighbor di ruang laten (konsisten dengan get_eval),
        bukan fallback frozen-decoder. Lihat decode_num_from_embedding.
    ...
    num_pred_norm = decode_num_from_embedding(
        ...
        ref_embeddings=ref_embeddings, ref_num_norm=ref_num_norm,
...
def save_imputed_csv_mrmd(dataname, pred_X, mask, split_df_path, save_path,
    ...
    ref_embeddings, ref_all_idx, ref_num_norm : opsional — [VIME] jika
        diberikan (train_ref_emb/train_ref_all_idx/train_ref_num atau
        test_ref_emb/test_ref_all_idx/test_ref_num dari load_dataset),
        decode kategorikal & numerik memakai Nearest-Neighbor di ruang
        laten — KONSISTEN dengan decode yang dipakai get_eval (bukan
        fallback frozen-decoder yang deterministik). Untuk DAE/PT-VAE,
        biarkan None (perilaku sama seperti sebelumnya).
    ...
    if len(cat_col_idx) > 0 and emb_model is not None:
        ...
        pred_all_idx = decode_cat_from_embedding(
            ...
            ref_embeddings=ref_embeddings, ref_all_idx=ref_all_idx,
        )  # [N, n_num_cols + n_cat_cols]
